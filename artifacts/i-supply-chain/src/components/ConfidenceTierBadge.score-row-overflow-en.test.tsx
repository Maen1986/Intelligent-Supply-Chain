/**
 * Task 810 — English evidence badge score row: no overflow at ≤360px
 *
 * Mirrors ConfidenceTierBadge.score-row-overflow.test.tsx (Task 807 Arabic
 * version) but runs entirely in English mode (lang="en").
 *
 ...
 *   </div>
 *
 * The English tier labels (Self-reported, AI-evaluated, Consultant-validated)
 * share the same score row as their Arabic counterparts.  If `flex-wrap` were
 * accidentally removed the English layout would overflow just as badly at
 * narrow widths (mobile grid, ≤360px).
 *
 * Section 1 — renders the real Maturity component in English results state and
 ...
 * to confirm that no English badge pill carries CSS classes that would clip
...
/** Render the real Maturity component in English results state. */
function renderMaturityEnglishResults(score = 3) {
...
/* ══════════════════════════════════════════════════════════════════════════
   Section 1 — Real Maturity component: score row carries flex-wrap (English)
   Renders the actual production code path in English mode; asserts the DOM
   node that Maturity.tsx produces for the score row has the flex-wrap class.
══════════════════════════════════════════════════════════════════════════ */

describe('Maturity results page — real score row carries flex-wrap in English (production render)', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.stubGlobal(
      'fetch',
  ...
  it('score-row-0 in the real English Maturity render has the flex-wrap class', () => {
    renderMaturityEnglishResults(3);
  ...
  it('every segment score row in the real English render carries flex-wrap', () => {
    renderMaturityEnglishResults(3);
  ...
  it('the Edit button is a sibling of the score row in English mode', () => {
    renderMaturityEnglishResults(3);
...
/* ══════════════════════════════════════════════════════════════════════════
   Section 2 — Direct ConfidenceTierBadge: all three English labels present
...
describe('ConfidenceTierBadge — all three English labels render (direct component render)', () => {
  afterEach(() => cleanup());

  it('renders "Self-reported" for self_reported in English mode', () => {
    render(<ConfidenceTierBadge lang="en" evidence={SELF_REPORTED_EV} asPill />);
    expect(screen.getByText('Self-reported')).toBeInTheDocument();
  });

  it('renders "AI-evaluated" for ai_evaluated in English mode', () => {
    render(<ConfidenceTierBadge lang="en" evidence={AI_EVALUATED_EV} asPill />);
    expect(screen.getByText('AI-evaluated')).toBeInTheDocument();
  });

  it('renders "Consultant-validated" for consultant_validated in English mode', () => {
    render(<ConfidenceTierBadge lang="en" evidence={CONSULTANT_VALIDATED_EV} asPill />);
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   clip text without a layout error.  Guard all three English tiers.
══════════════════════════════════════════════════════════════════════════ */

describe('ConfidenceTierBadge — no clipping class on English pill (direct render)', () => {
  afterEach(() => cleanup());

  it('self_reported pill has no overflow-hidden or truncate class', () => {
    render(<ConfidenceTierBadge lang="en" evidence={SELF_REPORTED_EV} asPill />);
    const badge = screen.getByText('Self-reported');
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   English label would force silent text clipping.  Pills must size to
   their content so they can wrap naturally inside the flex-wrap row.
══════════════════════════════════════════════════════════════════════════ */

describe('ConfidenceTierBadge — no fixed-width class on English pill (direct render)', () => {
  afterEach(() => cleanup());
