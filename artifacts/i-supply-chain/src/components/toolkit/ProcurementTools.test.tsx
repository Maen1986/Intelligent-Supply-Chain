...
describe('ProcurementToolsSection — arrow-key tab navigation (LTR)', () => {
  ...
  it('ArrowRight advances through all tabs sequentially', () => {
    render(<ProcurementToolsSection isAr={false} />);
    const tabs = screen.getAllByRole('tab');
    // Start at tab 0, press right 8 times → should land on tab 8 (last)
    for (let i = 0; i < 8; i++) {
...
describe('ProcurementToolsSection — arrow-key tab navigation (Arabic / RTL)', () => {
  ...
  it('ArrowRight advances through all 9 tabs in Arabic mode', () => {
    render(<ProcurementToolsSection isAr={true} />);
    const tabs = screen.getAllByRole('tab');
  ...
  it('full round-trip: ArrowRight × 9 returns to the starting tab in Arabic mode', () => {
    render(<ProcurementToolsSection isAr={true} />);
    const tabs = screen.getAllByRole('tab');
    for (let i = 0; i < 9; i++) {
      const current = tabs.findIndex(t => t.getAttribute('aria-selected') === 'true');
  ...
  it('full round-trip: ArrowLeft × 9 returns to the starting tab in Arabic mode', () => {
    render(<ProcurementToolsSection isAr={true} />);
    const tabs = screen.getAllByRole('tab');
    for (let i = 0; i < 9; i++) {
      const current = tabs.findIndex(t => t.getAttribute('aria-selected') === 'true');
