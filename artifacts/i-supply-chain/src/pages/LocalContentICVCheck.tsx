                ...
                {entry.countrySelection === 'SA' && entry.program === 'sa-iktva-aramco' && (
                  <div className="grid sm:grid-cols-2 gap-3">
                    ...
                    />
                                ...
                                {entry.countrySelection === 'SA' && entry.program === 'sa-rawafed-stc' && (
                  <div className="grid sm:grid-cols-2 gap-3">
                    <NumberField label={isAr ? 'محلي: السلع والخدمات' : 'Local: Goods & Services'} unit={isAr ? 'ر.س' : 'SAR'} value={entry.rawafedStc.localGoodsServicesSAR} onChange={v => onUpdate(entry.id, { rawafedStc: { ...entry.rawafedStc, localGoodsServicesSAR: v } })} />
                    <NumberField label={isAr ? 'إجمالي: السلع والخدمات' : 'Total: Goods & Services'} unit={isAr ? 'ر.س' : 'SAR'} value={entry.rawafedStc.totalGoodsServicesSAR} onChange={v => onUpdate(entry.id, { rawafedStc: { ...entry.rawafedStc, totalGoodsServicesSAR: v } })} />
                    <NumberField label={isAr ? 'محلي: الرواتب' : 'Local: Salaries'} unit={isAr ? 'ر.س' : 'SAR'} value={entry.rawafedStc.localSalariesSAR} onChange={v => onUpdate(entry.id, { rawafedStc: { ...entry.rawafedStc, localSalariesSAR: v } })} />
                    <NumberField label={isAr ? 'إجمالي: الرواتب' : 'Total: Salaries'} unit={isAr ? 'ر.س' : 'SAR'} value={entry.rawafedStc.totalSalariesSAR} onChange={v => onUpdate(entry.id, { rawafedStc: { ...entry.rawafedStc, totalSalariesSAR: v } })} />
                    <NumberField label={isAr ? 'محلي: إهلاك الأصول' : 'Local: Asset Depreciation'} unit={isAr ? 'ر.س' : 'SAR'} value={entry.rawafedStc.localAssetDepreciationSAR} onChange={v => onUpdate(entry.id, { rawafedStc: { ...entry.rawafedStc, localAssetDepreciationSAR: v } })} />
                    <NumberField label={isAr ? 'إجمالي: إهلاك الأصول' : 'Total: Asset Depreciation'} unit={isAr ? 'ر.س' : 'SAR'} value={entry.rawafedStc.totalAssetDepreciationSAR} onChange={v => onUpdate(entry.id, { rawafedStc: { ...entry.rawafedStc, totalAssetDepreciationSAR: v } })} />
                    <NumberField label={isAr ? 'محلي: تطوير القدرات' : 'Local: Capacity Development'} unit={isAr ? 'ر.س' : 'SAR'} value={entry.rawafedStc.localCapacityDevelopmentSAR} onChange={v => onUpdate(entry.id, { rawafedStc: { ...entry.rawafedStc, localCapacityDevelopmentSAR: v } })} />
                    <NumberField label={isAr ? 'إجمالي: تطوير القدرات' : 'Total: Capacity Development'} unit={isAr ? 'ر.س' : 'SAR'} value={entry.rawafedStc.totalCapacityDevelopmentSAR} onChange={v => onUpdate(entry.id, { rawafedStc: { ...entry.rawafedStc, totalCapacityDevelopmentSAR: v } })} />
                  </div>
                )}

                {entry.countrySelection === 'SA' && entry.program === 'sa-sabic-lc-commitment' && (
                  <div className="space-y-3">
                    <p className="text-[10px] text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-2 leading-relaxed flex items-start gap-1.5">
                      ...
                      {isAr
                        ? 'هذا ليس معياراً معلَناً على مستوى سابك ككل -- الممارسة الفعلية لسابك هي التزام محتوى محلي متفاوض عليه لكل عقد على حدة، يُدقَّق مقابل الأداء الفعلي المُسلَّم، وليس نسبة ثابتة على مستوى الشركة. هامش التسامح ونسبة الغرامة أدناه إعدادات افتراضية خاصة بقواعد عمل هذه المنصة، وليسا معياراً معلَناً من سابك.'
                        : "This is NOT a published SABIC-wide local-content standard -- SABIC's real practice is a per-contract negotiated target audited against actual delivered performance, never a fixed company-wide rate. The tolerance and penalty figures below are this platform's own disclosed business-rule defaults, not a published SABIC standard."}
                    </p>
                    <div className="grid sm:grid-cols-2 gap-3">
                      <NumberField
                        label={isAr ? 'الهدف المتفاوض عليه لهذا العقد' : "This Contract's Negotiated Target"}
                        unit="%"
                        max={100}
                        value={entry.sabicLcCommitment.proposedTargetPct}
                        onChange={v => onUpdate(entry.id, { sabicLcCommitment: { ...entry.sabicLcCommitment, proposedTargetPct: v } })}
                      />
                      <NumberField
                        label={isAr ? 'الأداء المدقَّق فعلياً' : 'Actual Audited Performance'}
                        hint={isAr ? `هامش التسامح: ${SABIC_LC_DEVIATION_TOLERANCE_PCT} نقطة مئوية` : `Tolerance band: ${SABIC_LC_DEVIATION_TOLERANCE_PCT} percentage points`}
                        unit="%"
                        max={100}
                        value={entry.sabicLcCommitment.actualAuditedPct}
                        onChange={v => onUpdate(entry.id, { sabicLcCommitment: { ...entry.sabicLcCommitment, actualAuditedPct: v } })}
                      />
                    </div>
                  </div>
                )}
