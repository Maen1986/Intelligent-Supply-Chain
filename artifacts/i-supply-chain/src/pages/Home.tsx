...
function useHeroFitBox(sectionRef: React.RefObject<HTMLElement | null>) {
  ...
  useEffect(() => {
    function measure() {
      ...
      // Eleventh pass (2 Sep 2026, owner's final call after trying every
      // other combination): three properties -- full width, zero crop, zero
      // scroll -- cannot all hold at once when the window is too short for
      // the image's natural height at full width; something has to give.
      // Width-narrowing (navy bars on the sides on a short window) is the
      // one the owner accepted this time -- explicitly preferred over both
      // cropping real infographic content (tenth pass) and over letting the
      // page scroll. So: on an ordinary viewport (the vast majority of
      // traffic) naturalHeight <= available and width stays full viewport
      // width, identical to every normal case before. Only when the window
      // is unusually short does width shrink below viewportWidth (bars
      // appear), height is capped to `available`, and the image is never
      // cropped (object-fit:contain always) and the page never scrolls.
      const cropped = false;
      const height = Math.min(naturalHeight, available);
      const width = naturalHeight <= available ? viewportWidth : height * HERO_ASPECT_RATIO;
      setBox(prev => (prev && prev.width === width && prev.height === height && prev.cropped === cropped)
        ? prev
